                            ...
                            @foreach($goods as $v)
                                <article class="media">
                                    <div class="media-body">
                                        <a class=" p-head" href="{{url('/goods/details/'.$v['gid'])}}">{{$v['gname']}}</a>
                                        <div class="" style="display: inline-block;float: right;padding: 8px;"><a href="javascript:;" onclick="DelUser({{$v['gid']}})">删除</a></div>
                                        <hr>
                                    </div>
                                </article>
                            @endforeach
        ...
        <script>
            function DelUser(nid){
                ...
                layer.confirm('是否确认删除？', {
                    ...
                    $.post("{{url('myfishpond/ytgoodsdel')}}",{'_token':"{{csrf_token()}}",nid:nid,yid:'{{$yt['yid']}}'},function(data){
                        if(data.status == 0){
                            layer.msg(data.msg, {icon: 6});
                            location.href = "{{url('/myfishpond/managegoods?yid=').$yt['yid']}}";
                        }else{
                            location.href = location.href;
